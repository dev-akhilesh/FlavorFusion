...
const Navbar = () => {
    ...
    const handleExploreClick = () => {
        ...
        const recipesSection = document.getElementById('recipes'); // Get the element by ID
        if (recipesSection) {
            recipesSection.scrollIntoView({ behavior: 'smooth' }); // Scroll to the element
        }
    };
        ...
        <header className='w-full fixed z-10 bg-black opacity-90'>
            <nav className='flex w-full py-2 md:py-3 px-4 md:px-20 items-center justify-between'>
                ...
                <ul className='hidden md:flex text-white gap-6'>
                    ...
                    <li>
                        <a href="#recipes" onClick={handleExploreClick}>Explore</a>
                    </li>
                    <li>
            ...
            <div className={`${open ? "flex" : "hidden"} bg-black flex-col w-full px-4 pt-16 pb-10 text-white gap-6 text-[14px]`}>
                <Link to="/">Home</Link>
                <a href="#recipes" onClick={handleExploreClick}>Recipes</a>
                <Link to="/favorites">Favorites</Link>
            </div>
        </header>
    );
